...
<head>
    ...
    <link href="servicos/css/bootstrap.min.css" rel="stylesheet" media="screen"> <!-- Usar estilo que o lojista desejar -->
    <link rel="stylesheet" type="text/css" href="servicos/css/style.css"> <!-- Usar estilo que o lojista desejar -->
    
    <script type="text/javascript" src="servicos/js/jquery.js"></script>
    <script type="text/javascript" src="servicos/js/bootstrap.min.js"></script> <!-- Usar estilo que o lojista desejar -->
    <script type="text/javascript" src="servicos/js/jquery.mask.js"></script>
    <script type="text/javascript" src="servicos/js/jquery.mask.min.js"></script>
    <script type="text/javascript" src="servicos/js/jquery.validate.min.js"></script>
    <script type="text/javascript" src="servicos/js/bootstrap-tooltip.js"></script>
    <script type="text/javascript" src="servicos/js/bootstrap-popover.js"></script>
...
<div class='container'>
    <form id='formCartao' action='PaybrasExemploRetorno.php' method="post">
        ...
        <input type='hidden' name='pagador_nome' value='<?php echo $_POST["pagador_nome"]; ?>'> <!-- Obrigatório --> <!-- Nome do cliente -->
        <input type='hidden' name='pagador_email' value='<?php echo $_POST["pagador_email"]; ?>'> <!-- Obrigatório --> <!-- Email do cliente -->
        <input type='hidden' name='pagador_cpf' value='<?php echo $_POST["pagador_cpf"]; ?>'> <!-- Obrigatório --> <!-- CPF do cliente -->
        <input type='hidden' name='pagador_rg' value='<?php echo $_POST["pagador_rg"]; ?>'> <!-- RG do cliente -->
        <input type='hidden' name='pagador_telefone_ddd' value='<?php echo $_POST["pagador_telefone_ddd"]; ?>'> <!-- Obrigatório --> <!-- DDD do cliente -->
        <input type='hidden' name='pagador_telefone' value='<?php echo $_POST["pagador_telefone"]; ?>'> <!-- Obrigatório --> <!-- Telefone do cliente -->
        <input type='hidden' name='pagador_celular_ddd' value='<?php echo $_POST["pagador_celular_ddd"]; ?>'><!-- DDD do celular do cliente -->
        <input type='hidden' name='pagador_celular' value='<?php echo $_POST["pagador_celular"]; ?>'> <!-- Celular do cliente -->
        <input type='hidden' name='pagador_sexo' value='<?php echo $_POST["pagador_sexo"]; ?>'> <!-- Sexo do cliente (M ou F) -->
        ...
        <input type='hidden' name='entrega_complemento' value='<?php echo $_POST["entrega_complemento"]; ?>'> <!-- Complemento do Endereço de entrega -->
        <input type='hidden' name='entrega_bairro' value='<?php echo $_POST["entrega_bairro"]; ?>'> <!-- Obrigatório --> <!-- Bairro de entrega -->
        <input type='hidden' name='entrega_cep' value='<?php echo $_POST["entrega_cep"]; ?>'> <!-- Obrigatório --> <!-- CEP de entrega -->
        <input type='hidden' name='entrega_cidade' value='<?php echo $_POST["entrega_cidade"]; ?>'> <!-- Obrigatório --> <!-- Cidade de entrega -->
        <input type='hidden' name='entrega_estado' value='<?php echo $_POST["entrega_estado"]; ?>'> <!-- Obrigatório --> <!-- Estado de entrega -->
        <input type='hidden' name='entrega_pais' value='<?php echo $_POST["entrega_pais"]; ?>'> <!-- Obrigatório --> <!-- País de entrega -->

        <input type='hidden' name='pagador_logradouro' value='<?php echo $_POST["pagador_logradouro"]; ?>'> <!-- Endereço do cliente -->
        <input type='hidden' name='pagador_numero' value='<?php echo $_POST["pagador_numero"]; ?>'> <!-- Número do Endereço do cliente -->
        <input type='hidden' name='pagador_bairro' value='<?php echo $_POST["pagador_bairro"]; ?>'> <!-- Bairro do cliente -->
        <input type='hidden' name='pagador_cep' value='<?php echo $_POST["pagador_cep"]; ?>'> <!-- CEP do cliente -->
        <input type='hidden' name='pagador_cidade' value='<?php echo $_POST["pagador_cidade"]; ?>'> <!-- Cidade do cliente -->
        <input type='hidden' name='pagador_estado' value='<?php echo $_POST["pagador_estado"]; ?>'> <!-- Estado do cliente -->
        <input type='hidden' name='pagador_pais' value='<?php echo $_POST["pagador_pais"]; ?>'> <!-- País do cliente -->

        <input type='hidden' name='pedido_id' value='<?php echo $_POST["pedido_id"]; ?>'> <!-- Obrigatório --> <!-- ID do pedido (armazenado na loja) -->
        <input type='hidden' name='pedido_valor_total_original' value='<?php echo $_POST["pedido_valor_total_original"]; ?>'> <!-- Obrigatório --> <!-- Valor da Compra (produtos + frete) -->
        <input type='hidden' name='pedido_descricao' value='<?php echo $_POST["pedido_descricao"]; ?>'> <!-- Descrição do pedido -->
        <input type='hidden' name='pedido_moeda' value='BRL'> <!-- Aceito apenas Real --> <!-- Obrigatório -->
        <input type='hidden' name='pedido_url_redirecionamento' value='<?php echo $_POST["pedido_url_redirecionamento"]; ?>'> <!-- Obrigatório para TEF --> <!-- URL do retorno após pagamento com débito em conta -->
        ...
        if(isset($_POST['recebedores']) && !empty($_POST['recebedores'])){
            foreach ($_POST['recebedores'] as $key => $value) { ?>
                <input type='hidden' name='recebedores[<?php echo $key;?>][recebedor_email]' value='<?php echo $value['recebedor_email'];?>'> <!-- Email do recebedor secundário-->
                <input type='hidden' name='recebedores[<?php echo $key;?>][recebedor_valor]' value='<?php echo $value['recebedor_valor'];?>'> <!-- Valor atribuido ao recebedor secundário -->
        <?php }
        } ?>
        ...
        <div id="card" class="payMethod hide">
            ...
            <div class='span3'>
                ...
                <?php 
                    require_once "servicos/PaybrasPostParcelas.php";

                    $pedido_valor = $_POST['pedido_valor_total_original']; //Valor da Compra (produtos + frete)
